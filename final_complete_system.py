...
for agent_class, agent_spec in zip(agent_classes, agent_specs):
    ...
    if agent_type == "keras-rl":
        ...
        # Agent Train
        agent.compile(Adam(lr=1e-2), metrics=['mse'])
        history = agent.fit(env, nb_steps=EPISODES*500, nb_max_episode_steps=1000, visualize=False, verbose=2)
...
for agent_class, agent_spec in zip(agent_classes, agent_specs):
    ...
    if agent_type == "keras-rl":
        ...
        # Agent Train
        agent.compile(Adam(lr=1e-2), metrics=['mse'])
        history = agent.fit(env, nb_steps=EPISODES*1000, nb_max_episode_steps=1000, visualize=False, verbose=2)
